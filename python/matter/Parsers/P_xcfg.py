...
__id__ = "$Id: P_xcfg.py 3589 2009-09-09 17:47:24Z juhas $"
...
class P_xcfg(StructureParser):
        ...
        try:
            ...
            for line in ilines:
                ...
                if stripped_line == "" or line[0] == '#':
                    ...
                    if line.find("Number of particles =") != 0:
                        emsg = ("{0!d}: first line must " +
                                "contain 'Number of particles ='").format(p_nl)
            ...
            for i in range(p_auxnum):
                if i not in p_auxiliary:
                    p_auxiliary[i] = "aux{0!d}".format(i)
            ...
            if 6-3*xcfg_NO_VELOCITY+len(p_auxiliary) != xcfg_entry_count:
                emsg = ("{0!d}: auxiliary fields " +
                        "not consistent with entry_count").format(p_nl)
                raise StructureFormatError(emsg)
            ...
            for idx in sorted_aux_keys:
                ...
                if prop == "Uiso":
                    p_exprs.append("a.U[0,0]=a.U[1,1]=a.U[2,2]=" +
                        "fields[{0!d}]".format(col))
                elif re.match(r"^U\d\d$", prop) \
                and 1 <= int(prop[1]) <= 3 and 1 <= int(prop[2]) <= 3:
                    i, j = int(prop[1])-1, int(prop[2])-1
                    if i == j:
                        p_exprs.append("a.U[{0!i},{1!i}]=fields[{2!d}]".format(i, j, col))
                    else:
                        p_exprs.append("a.U[{0!i},{1!i}]=a.U[{2!i},{3!i}]=fields[{4!d}]".format((i, j, j, i, col)))
                else:
                    p_exprs.append("a.__dict__[{0!r}]=fields[{1!d}]".format(prop, col))
            p_assign_expr = "pass; " + "; ".join(p_exprs[3:])
            exec("def p_assign_atom(a, fields) : {0!s}".format(p_assign_expr))
            ...
            for line in lines[p_nl:stop]:
                ...
                # parse element allowing empty symbol
                    ...
                    emsg = "{0!d}: invalid record".format(p_nl)
                    raise StructureFormatError(emsg)
            if len(stru) != p_natoms:
                emsg = "expected {0!d} atoms, read {1!d}".format(p_natoms, len(stru))
                raise StructureFormatError(emsg)
        except (ValueError, IndexError):
            emsg = "{0!d}: file is not in XCFG format".format(p_nl)
    ...
    def toLines(self, stru, **kwds):
        ...
        if len(stru) == 0:
            ...
            raise StructureFormatError(emsg)
        lines = []
        lines.append("Number of particles = {0:i}".format(len(stru)))
        ...
        lines.append("A = {0:.8g} Angstrom".format(p_A))
        ...
        for i in range(3):
            for j in range(3):
                lines.append("H0({0:i},{1:i}) = {2:.8g} A".format(i+1, j+1, stru.lattice.base[i, j]))
        ...
        lines.append("entry_count = {0!d}".format(p_entry_count))
        # add auxiliaries
        for i in range(len(p_auxiliaries)):
            lines.append("auxiliary[{0!d}] = {1!s} [au]".format(i, p_auxiliaries[i][0]))
        ...
        p_exprs += ["    fields += [ " +
                    ...
                    "    line = ' '.join([ '{0:.8g}'.format(x for x in fields ])",
                    "    return line"]
        exec("\n".join(p_exprs))
        ...
        for a in stru:
            if a.symbol != p_symbol:
                p_symbol = a.symbol
                lines.append("{0:.4f}".format(AtomicMass.get(p_symbol, 0.0)))
